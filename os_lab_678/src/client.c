...
void client_loop() {
	for (;;) {
		...
		// back to server msg
		if (msg.to_id == -1) {
			// printf("[%d] Client %d: resending msg to top throuth \"%s\"\n", CLIENT_PID, CLIENT_ID, PARENT_PUB_ENDPOINT);
			// fflush(stdout);
		...
		if (msg.cmd == create) {
			...
			if (msg.to_id < CLIENT_ID && HAS_LEFT) {
				send_msg(LEFT_PUB, &msg);
				// printf("[%d] Client %d: send to left\n", CLIENT_PID, CLIENT_ID); fflush(stdout);
			} else
			if (msg.to_id > CLIENT_ID && HAS_RIGHT) {
				send_msg(RIGHT_PUB, &msg);
		...
		} else if (msg.cmd == timer_start) {

			gettimeofday(&START, NULL);
			msg.to_id = -1;
			send_msg(PARENT_PUB, &msg);
		} else if (msg.cmd == timer_stop) {

			gettimeofday(&END, NULL);
			msg.to_id = -1;
			send_msg(PARENT_PUB, &msg);
		} else if (msg.cmd == timer_display) {

			int sec = END.tv_sec - START.tv_sec;
			int microsec = END.tv_usec - START.tv_usec;
			if (microsec < 0) {
				--sec;
				microsec += 1000000;
			}
			int elapsed = sec*1000 + microsec/1000;
			msg.to_id = -1;
			msg.value = elapsed;
			send_msg(PARENT_PUB, &msg);
