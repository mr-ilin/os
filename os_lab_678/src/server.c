...
void send_exec_msg( const int to_id, const cmd_type type) {
    message msg;
...
// shall return pid on success, either 0
...
void create_child_process(const int client_id) {
    if (client_id < 1) {
...
void exec_command(const int client_id, const cmd_type cmd) {
    if (!ping_client(client_id)) {
        fprintf(stderr, "[%d] Error: not found\n", SERVER_PID);
        return;
    }

    send_exec_msg(client_id, cmd);
    message msg;
    if (!get_msg(SUBSCRIBER, &msg)) {
    ...
    if (msg.to_id == -1 && msg.cmd == cmd) {
        if (cmd == timer_display) {
            printf("[%d] OK:%d: %d\n", SERVER_PID, client_id, msg.value);
        } else {
            printf("[%d] OK:%d\n", SERVER_PID, client_id);
        }
        return;
    } 
...
void input_loop() {
    ...
    for (;;) {
        ...
        if (!strcmp(cmd, "exec")) {
            ...
            char exec_cmd[MAX_LEN];
            int scanf_res = scanf("%s", exec_cmd);
            if (scanf_res == EOF) {
                printf("[%d] Shutting down server.\n", SERVER_PID);
                break;
            } else if (scanf_res == 0) {
            ...
            cmd_type type;
            if (!strcmp(exec_cmd, "start")) {
                type = timer_start;
            } else
            if (!strcmp(exec_cmd, "stop")) {
                type = timer_stop;
            } else
            if (!strcmp(exec_cmd, "time")) {
                type = timer_display;
            } else {
                printf("[%d] Invalid exec command\n", SERVER_PID);
                continue;
            }
            exec_command(id, type);
        } else 
        if (!strcmp(cmd, "ping")) {
